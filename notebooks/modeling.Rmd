...
# 10. Metric Comparison Plot (Display Only)

```{r metric-comparison, echo=TRUE, message=FALSE, warning=FALSE}
################################################################################
#                     METRIC COMPARISON PLOT
# ------------------------------------------------------------------------------
# Shows side-by-side comparison of Accuracy, Sensitivity, Specificity,
# Precision, F1, and AUC for RF and SVM on the TEST SET.
################################################################################
...
ggplot(metric_long, aes(x = Metric, y = Value, fill = Model)) +
  ...
  labs(
    title = "Performance Metrics Comparison: Random Forest vs SVM (Test Set)",
    x = "Metric",
    y = "Score"
  ) +
  scale_fill_manual(values = c("RF" = "steelblue", "SVM" = "firebrick")) +
  ylim(0, 1)
```